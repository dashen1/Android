package com.example.kotlin.data.remote.moshiFactories
...
/**
 * Placeholder value used when a field is absent from JSON. Note that this code
...
/**
 * This class encodes Kotlin classes using their properties. I decodes them by first invoking the constructor,
 * and then by setting any additional properties that exist, if any.
 */
...
internal class MyKotlinJsonAdapter<T>(
    ...
    override fun fromJson(reader: JsonReader): T? {
        ...
        // Read each value into its slot int the array.
        ...
        while (reader.hasNext()) {
            ...
            if (values[index] !== ABSENT_VALUE) {
                throw JsonDataException("Multiple values for '${constructor.parameters[index].name}' at ${reader.path}")
            }
        ...
        for (i in 0 until constructorSize) {
            if (values[i] === ABSENT_VALUE && !constructor.parameters[i].isOptional) {
                if (!constructor.parameters[i].type.isMarkedNullable) {
                    throw JsonDataException("Required value '${constructor.parameters[i].name}' at ${reader.path}")
                }
                values[i] = null
            }
        }
        // Call the constructor using a Map so that absent optional get default.
...
class MyKotlinJsonAdapterFactory : JsonAdapter.Factory {
    ...
    override fun create(
        ...
        try {
            val generateAdapter = generatedAdapter(moshi, type, rawType)
            ...
            // Fall back to reflective adapter when the generated adapter is not found.
        }
        ...
        for (property in rawTypeKotlin.memberProperties) {
            ...
            if (Modifier.isTransient(property.javaField?.modifiers ?: 0)) {
                if (parameter != null && !parameter.isOptional) {
                    throw IllegalArgumentException("No default value for transient constructor $parameter")
                }
                continue
            }
        ...
        val options = JsonReader.Options.of(*bindings.map { it?.name ?: "\u00000" }.toTypedArray())
        return MyKotlinJsonAdapter(constructor, bindings, options).nullSafe()
    }
