 ...
 * non-zero it is not idle. This is very similar to the way a [java.util.concurrent.Semaphore] behaviors.
 *
...
class SimpleCountingIdlingResource(private val resourceName: String) : IdlingResource {
    ...
    fun decrement(){
        ...
        if (counterVal==0){
            // we've gone from non-zero to zero,That means we're Idle now! Tell the espresso.
